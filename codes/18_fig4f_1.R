...
pheno = pheno[, c('Line1', 'alpD')]
pheno = pheno %>% group_by(Line1) %>% summarise(Dominants = mean(alpD))
...
write.csv(pheno, 'results/dominantsForGWAS.csv')

writer = function(x, i) {
  df = x[i, ]
  write.table(df,
              paste0('results/inputForGWAS/dominants/dominants', as.character(numbers[n]), '.csv'),
              quote = TRUE, sep = ',',
