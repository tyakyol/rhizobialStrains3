...
# Alpha diversity
set.seed(25); alpG = sample(vegan::diversity(t(GG), index = 'shannon'), size = 603, replace = F)
set.seed(100); alpD = sample(vegan::diversity(t(DD), index = 'shannon'), size = 603, replace = F)
set.seed(400); alpS = sample(vegan::diversity(t(SS), index = 'shannon'), size = 603, replace = F)
...
mergD = cbind(mergD, alpG, alpD, alpS)
# write.csv(mergD, 'results/shannon.csv')
...
saveRDS(res1_,  'results/growthPredictionWithDiversity/randomShannon1.rds')
saveRDS(res1__, 'results/growthPredictionWithDiversity/randomShannon2.rds')
