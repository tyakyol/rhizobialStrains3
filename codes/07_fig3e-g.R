...
df = data.frame(num = unlist(outForComp),
                bac = rep(names(outForComp), each = 56))
...
ggsave('results/07_extendedFig3c.pdf', height = 5, width = 7)
...
pdf(file = 'results/07_fig3e.pdf', useDingbats = F, width = 5, height = 5)
par(mar = c(0,0,0,0))
plot(gr,
...
pdf(file = 'results/07_fig3g.pdf', useDingbats = F, width = 6, height = 6)
