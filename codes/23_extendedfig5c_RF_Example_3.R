# This code is an example of the RF runs on GWAS results (randomly chosen 200 markers) for Dominants. We ran the same code for 100 different partitions.
# Output is prediction accuracy.
...
pheno = read.delim('results/dominantsRFCRA.csv', sep = ',')
pheno[,'Dominants'] = log2(pheno[,'Dominants'] + 1e-6)
res = read.delim('results/gwas99429DominantsCRA.csv', sep = ',')
...
set.seed(sss); snps2 = snps[res[, 'Label'][sample(201:nrow(res), size = 200)],]
snps2 = snps2[,3:214]
