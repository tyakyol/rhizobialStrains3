...
# Calculate diversity
div = as.data.frame(vegan::diversity(t(dfRA)))

# Merge with biomass
div$Plant = colnames(dfRA)
colnames(div)[1] = 'Diversity'
div = left_join(div, lf, by = 'Plant')
div$Comb = paste(div$Ref, div$Line1, sep = '_')
pgR$Comb = paste(pgR$Ref, pgR$Line1, sep = '_')

fff = left_join(div, pgR, by = 'Comb')
fff = fff[complete.cases(fff), ]
fff$Diversity = as.numeric(scale(fff$Diversity))

# Result
m = lmerTest::lmer(newBM ~ Diversity + (1 | Line1.x) + (1 | Batch) + V2 + V3 + V4, data = fff)
mm = summary(m)
