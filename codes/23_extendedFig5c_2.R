...
pheno = pheno[, c('Line1', 'raG')]
pheno = pheno %>% group_by(Line1) %>% summarise(Generalists = mean(raG))
...
write.csv(pheno, 'results/generalistsForGWAS_CRA.csv')

writer = function(x, i) {
  df = x[i, ]
  write.table(df,
              paste0('results/inputForGWAS_CRA/generalists/generalists', as.character(numbers[n]), '.csv'),
              quote = TRUE, sep = ',',
