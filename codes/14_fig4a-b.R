...
ggsave(filename = 'results/extendedFig2a.pdf', device = 'pdf', width = 6, height = 6)
...
qplot(mp$V3, mp$V4, geom = 'blank') +
  ...
  theme(panel.grid = element_blank(), 
        ...
        axis.title = element_text(size = 18),
        axis.ticks = element_blank())
ggsave(filename = 'results/extendedFig2b.pdf', device = 'pdf', width = 6, height = 6)
...
coef_estimates$term = factor(coef_estimates$term, levels = unique(coef_estimates$term))  # Hey!
saveRDS(coef_estimates, 'results/lmmWithMDS.rds')

# Plot for the abundance coefficients
...
ggsave(filename = 'results/14_fig4a.pdf', device = 'pdf', width = 6, height = 6)
...
ggplot(tabOut3, aes(x = class, y = estimate,
  ...
  theme(panel.grid = element_blank(), 
        ...
        axis.title = element_text(size = 18),
        axis.ticks = element_blank())
ggsave(filename = 'results/14_fig4b.pdf', device = 'pdf', width = 6, height = 6)
